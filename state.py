...
Sa a pèmèt nou swiv kote chak moun ye nan flow la (dekonpile -> konfigire
-> patch -> bwode), memwa ant mesaj yo.
"""
import json
import os
import threading
...
def _state_path(user_id) -> str:
    """Chemine fichye JSON pou yon itilizatè."""
    return os.path.join(STATE_DIR, f"{user_id}.json")


def get_state(user_id) -> dict:
    """Li estaj yon itilizatè. Retounen diksyonè (vid si pa egziste)."""
...
def set_state(user_id, state: dict):
    """Ekri estaj yon itilizatè sou disk (thread-safe)."""
...
def update_state(user_id, **kwargs):
    """Fizone nouvo kle nan estaj la, epi sove."""
...
def clear_state(user_id):
    """Efase estaj yon itilizatè (apre yon operasyon fini)."""
    p = _state_path(user_id)
    if os.path.exists(p):
        try:
            os.remove(p)
