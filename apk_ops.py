...
def decompile(apk_path: str, out_dir: str) -> str:
    """Dekonpile yon APK ak apktool. Retounen chemine reyèltif pwojè a.

    apktool kreye <out_dir>/<basename_san_ekstansyon>/.
    """
...
def rebuild(project_dir: str, out_apk: str) -> str:
    """Rekonstwi APK ak apktool. Retounen chemine APK ki pwodui."""
...
def zipalign(in_apk: str, out_apk: str) -> str:
    """Aliyen APK ak zipalign -f 4. Retounen chemine aliyen."""
...
def full_build_pipeline(project_dir: str, work_root: str, apk_base: str):
    """Egzekite rekonstwi + zipalign + siyen, retounen chemine final APK.

    Retounen (ok, apk_final_path, message).
    """
    try:
