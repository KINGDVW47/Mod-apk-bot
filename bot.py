...
# =============================================================================
(
    WAIT_APK,          # ap tann pou itilizatè voye yon .apk
    WAIT_NAME,         # ap tann non aplikasyon
    WAIT_EMAIL,        # ap tann imèl
    WAIT_PHONE,        # ap tann telefòn
    WAIT_PATCH_SELECT, # ap tann chwazi patche
) = range(5)

# Bouton "sote" pou chan opsyonèl
SKIP = "skip"
...
def _work_root(user_id: int) -> str:
    """Repèrtwar travay pwòp pou yon itilizatè."""
    root = os.path.join(config.WORK_DIR, str(user_id))
    os.makedirs(root, exist_ok=True)
    return root


async def _download_apk(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str:
    """Telechaje APK voye pa itilizatè a, retounen chemin lokal."""
...
async def _send_long(update: Update, text: str):
    """Voye yon mesaj (koupe an möso si twò long)."""
    MAX = 4000
    if len(text) <= MAX:
...
def _esc(text: str) -> str:
    """Echape karaktè HTML pou evite parse erè."""
    return (text.replace("&", "&amp;")
                 .replace("<", "&lt;")
                 .replace(">", "&gt;"))
...
async def receive_apk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Netwaye anyen ki te rete anvan
    safe_remove(_work_root(uid))
...
# =============================================================================
# Etap 4: Telefòn + montre seleksyon patch
# =============================================================================
...
async def toggle_patch(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Bazkar yon patch sou/soti lè itilizatè klike bouton an."""
...
# =============================================================================
# Konstwi ak aplike tout modifikasyon
# =============================================================================
...
async def _process_build(update: Update, context: ContextTypes.DEFAULT_TYPE, st: dict):
    ...
    # --- 2. Resous pèsonalize (mod_name, mod_email, mod_phone)
    custom = {}
    if st.get("new_name"):
    ...
    # --- 3. Enjekte Toast (toujou fèt si nou ka)
    toast_msg = f"Modded by APK Mod Bot"
    try:
    ...
    # Voye fichye — gen de fason. Nou itilize sendDocument.
...
def main():
    ...
    # Long polling — pa bezwen webhook
